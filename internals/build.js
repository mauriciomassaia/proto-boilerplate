...
const protos = fs
  .readdirSync('./src')
  .filter((item) => item.indexOf('proto-') > -1);
...
if (process.argv.indexOf('-all') > -1) {
  ...
  protosToBuild = builds.filter((num) =>
    protos.filter((id) => id === `proto-${num}`)
  );
...
const compiler = webpack(
  protosToBuild.map((ID) =>
    require('./webpack.base')({ ID, mode: 'production' })
  )
);
