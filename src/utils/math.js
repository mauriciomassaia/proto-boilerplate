...
export function angleBetweenPoints (v1, v2) {
  // angle in radians
  let angleRadians = Math.atan2(v2.y - v1.y, v2.x - v1.x)
  if (angleRadians < 0) {
...
export function lineSegmentsIntersect (pointAStart, pointAEnd, pointBStart, pointBEnd) {
  ...
  const det = determinant(vecSub(pointAEnd, pointAStart), vecSub(pointBStart, pointBEnd))
  const t = determinant(vecSub(pointBStart, pointAStart), vecSub(pointBStart, pointBEnd)) / det
  const u = determinant(vecSub(pointAEnd, pointAStart), vecSub(pointBStart, pointAStart)) / det
